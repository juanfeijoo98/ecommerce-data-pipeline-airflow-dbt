...
from airflow.operators.trigger_dagrun import TriggerDagRunOperator  # Permite disparar otro DAG desde este
...
def fetch_weather():
    ...
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")  # Timestamp actual en formato ISO
...
def validate_weather_json():
    ...
    assert "hourly" in payload, "No viene 'hourly' en la respuesta"  # Verifica que exista 'hourly'
    for key in ["time", "temperature_2m", "relative_humidity_2m"]:
        assert key in payload["hourly"], f"Falta '{key}' en hourly"  # Verifica claves necesarias
    n = len(payload["hourly"]["time"])  # Cuenta registros
    assert n == len(payload["hourly"]["temperature_2m"]) == len(payload["hourly"]["relative_humidity_2m"])  # Consistencia
...
def to_silver_parquet():
    ...
    ensure_dirs()  # Asegura que los directorios existen
    latest = sorted(BRONZE.glob("weather_*.json"))[-1]  # Busca el archivo JSON más reciente
    payload = json.loads(latest.read_text())  # Lee y carga el JSON
    ...
    if out.exists():
        ...
        df = pd.concat([old, df]).drop_duplicates(subset=["ts_utc"]).sort_values("ts_utc")
...
def load_to_postgres():
    ...
    con validaciones y mensajes claros.
    """
    ...
    assert src.exists(), f"[load_to_postgres] Parquet no existe: {src}. ¿Se ejecutó to_silver_parquet?"

    # 2) Leo el parquet (requiere pyarrow)
    try:
        df = pd.read_parquet(src)
    except Exception as e:
        raise RuntimeError(f"[load_to_postgres] Error leyendo parquet {src}: {e}")

    print(f"[load_to_postgres] filas a cargar: {len(df)}")

    # 3) Conexión a Postgres del compose (host=postgres)
    ...
    try:
        ...
        raise RuntimeError(f"[load_to_postgres] Error conectando a Postgres ({url}): {e}")

    # 4) Carga con to_sql (chunks para no matar memoria)
    try:
        df.to_sql(
            "weather_hourly",
            engine,
        ...
        raise RuntimeError(f"[load_to_postgres] Error escribiendo en public.weather_hourly: {e}")

    # 5) Verificación rápida
    from sqlalchemy import text as sql_text
    with engine.begin() as conn:
        count = conn.execute(sql_text("SELECT COUNT(*) FROM public.weather_hourly;")).scalar()
...
with DAG(
    ...
    t_check    = PythonOperator(task_id="validate_weather_json",  python_callable=validate_weather_json)
    t_silver   = PythonOperator(task_id="to_silver_parquet",      python_callable=to_silver_parquet)
    t_pg       = PythonOperator(task_id="load_to_postgres",       python_callable=load_to_postgres)
    t_gold     = PythonOperator(task_id="to_gold_aggregates",     python_callable=to_gold_aggregates)
